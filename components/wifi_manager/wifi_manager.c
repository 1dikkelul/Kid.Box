                          ...
                          int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        s_status.started = true;        // <-- add
        s_status.connected = false;     // <-- add (optional)
        s_status.got_ip = false;        // <-- add (optional)
        ...
        s_status.ip = event->ip_info.ip.addr;   // <-- numeric storage of IP (optional, for easy printing without conversion)
...
{
    esp_ip4_addr_t a = { .addr = s_status.ip };
    esp_ip4addr_ntoa(&a, buf, buflen);
}

bool wifi_manager_is_ready(void)
{
    return s_status.got_ip;
}
